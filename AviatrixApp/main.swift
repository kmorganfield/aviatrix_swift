...
func fly(myPlane : Aviatrix) {
    ...
    for (index, city) in destinations.enumerated() {
        let distance = myPlane.distanceTo(location: plane.location, target: city)
...
func refuel(myPlane : Aviatrix) {
    ...
    print("⛽ You refueled \(plane.refuelAmount) gallons totaling \(plane.maxFuel)")
}
...
var plane = Aviatrix(programAuthor: "Kennedy")
